...
/* ===================== values ===================== */
typedef enum { V_NIL, V_BOOL, V_NUM, V_MONEY, V_STR, V_WALLET, V_FUNC, V_BUILTIN } VType;
struct Node; struct Env; struct Interp;
...
typedef enum { T_EOF, T_NUM, T_MONEY, T_STR, T_IDENT, T_KW, T_OP,
               T_LP, T_RP, T_LB, T_RB, T_COMMA, T_DOT } TokType;
...
static const char *KEYWORDS[] = {
  ...
  "price","wallet","pay","from","to","require","gas", NULL
};
...
static Node *equality(Parser *p){ const char *o[]={"==","!="}; return bin_lvl(p,comparison,o,2,0); }
...
static int starts_expr(Token *t){
  if(t->type==T_NUM||t->type==T_MONEY||t->type==T_STR||t->type==T_IDENT||t->type==T_LP) return 1;
...
int main(int argc, char **argv){
  ...
  for(int i=1;i<argc;i++){
    if(strcmp(argv[i],"--version")==0 || strcmp(argv[i],"-v")==0){ printf("larzscript (native) 0.1.0\n"); return 0; }
    if(strcmp(argv[i],"--help")==0 || strcmp(argv[i],"-h")==0){ printf("usage: larzscript <program.lz>\n"); return 0; }
    path=argv[i];
  }
